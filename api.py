...
def configure_ngrok():
    ngrok_token = "2pFo3S9Chr6s9Czhowaql79EK03_6JKYxRbHsRAk7WRaHPTip"
    try:
...
def update_soul_txt(public_url):
    with open("anony3.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony3.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony3.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony3.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony3.txt on VPS: {str(e)}")
